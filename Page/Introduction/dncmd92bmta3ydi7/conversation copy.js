...
function conversationRecode() {
    ...
    function 前言() {
        conversation.print(`${Character[0]}.working.gif`, '操作說明：你可以點擊對話框來取得資訊，並且使用滑鼠點擊');
        conversation.print(`${Character[0]}`, '操作說明：網頁有音效，預設是開啟的，但是部分瀏覽器會擋住需要手動開啟');
        conversation.print(`${Character[0]}.C110152351.gif`, '我是 C110152351 的代理機器人，你現在在 C110152351 的自傳裡');
    ...
    function 談話() {
        conversation.option(`${Character[0]}.lock.gif`, '好吧！那你得答應我，你不讓其他人知道這裡的存在', [
    ...
    function 那你想跟我談論什麼() {
        conversation.option(`${Character[0]}.questions.gif`, '那你想跟我談論什麼', [
    ...
    function 家庭狀況() {
        conversation.print(`${Character[0]}.working.gif`, '據我所知 C110152351 並不喜歡向別人談論他的家庭');
    ...
    function 興趣與成長() {
        conversation.print(`${Character[0]}.working.gif`, 'C110152351 從小就很喜歡電子產品');
    ...
    function 思想與信仰與人生觀() {
        conversation.print(`${Character[0]}.working.gif`, '嗯......我想我這樣解釋好了');
        conversation.print(`${Character[0]}`, '假設這裡有一個熱水器，他可能是用「金屬」、「電線」、「加熱管」等等以規律的方式組成');
        conversation.print(`${Character[0]}`, '當我不使用熱水時，我就不會在意它');
        conversation.print(`${Character[0]}`, '簡單來說，我們對事物的價值事實上取決於事物對我們的「影響」');
        conversation.print(`${Character[0]}`, '這些「影響」可能是「同情心」、「需求」、「依附感」等等來自「需求層次理論」的需求');
        conversation.print(`${Character[0]}`, '對我來說，這些來自環境與內心的「影響」會多次在我腦中辯論，進而改變我的身心及行為');
        conversation.print(`${Character[0]}`, '尤其是當「影響」被我認定為對我影響不大，我就會顯得有點虛無主義，反之');
        conversation.option(Character[0], '（沒了）', [
            { say: '繼續談話', goTo: '那你想跟我談論什麼' },
            { say: '離開', goTo: 'leave' },
        ]);
    ...
    function 其他() { }
    function leave() {
        conversation.print(`${Character[0]}.zzz.gif`, '( 休眠中......（●__●）  )');
        conversation.option(`${Character[0]}.zzz.gif`, '( 休眠中......（●__●）  )', [
            { say: '查看資料資源', goTo: '查看資料資源' },
        ]);
        return conversation.makeConversationParagraph();
    }
    function 查看資料資源() {
        conversation.print(`${Character[0]}.working.gif`, '背景音樂是在「こんとどぅふぇ公式ちゃんねるッ」的Youtube頻道找的');
        conversation.print(`${Character[0]}`, '音效是在「小森平的免費下載音效」找的');
        conversation.print(`${Character[0]}`, '角設是C110152351他自己在「Piskel」畫得');
        conversation.print(`${Character[0]}`, '操作系統、界面也是C110152351做的');
        conversation.print(`${Character[0]}`, '如果有Bug會其他問題可以到電子郵件找C110152351');
        conversation.print(`${Character[0]}`, '電子郵件是「c110152351@nkust.edu.tw」');
        conversation.option(`${Character[0]}`, '（沒了）', [
            { say: '離開', goTo: 'leave' },
        ]);
        return conversation.makeConversationParagraph();
